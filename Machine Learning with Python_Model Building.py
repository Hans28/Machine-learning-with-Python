...
mp=sns.FacetGrid(df,col="Gender",hue="loan_status")
mp.map(plt.hist,"Principal",ec='k')
plt.show()
...
## Daywise% loan sts
round(df['dayofweek'].value_counts(normalize=True)*100,2)
...
# visualization 1 (if you want to see what does this decision tree look like)
...
out=tree.export_graphviz(best_tree,feature_names=featureNames, out_file=dot_data, class_names= np.unique(y_trainset), filled=True,  special_characters=True,rotate=False)  
